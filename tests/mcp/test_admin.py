...
async def test_list_tools_advertises_admin_tools(state_dir: Path) -> None:
    ...
    assert names == {
        ...
        "admin_log_tail",
        "admin_log_grep",
        "admin_mods_installed",
        "admin_live_status",
        "admin_service_health",
        "admin_rcon_query",
    }
    assert all(name.startswith("admin_") for name in names)
    # admin_config_get exposes an enum of names, never a free-form path field.
    cfg_tool = next(t for t in result.root.tools if t.name == "admin_config_get")
    props = cfg_tool.inputSchema["properties"]
    assert "enum" in props["name"]
    assert "network" in props["name"]["enum"]
    rcon_tool = next(t for t in result.root.tools if t.name == "admin_rcon_query")
...
async def test_legacy_admin_tool_names_are_not_dispatched(state_dir: Path) -> None:
    ...
    assert _json_block(result)["message"] == "unknown tool 'eco_admin_save_status'"


@pytest.mark.asyncio
async def test_config_get_operator_strips_secrets_shows_names(state_dir: Path) -> None:
    result = await _call(StateStore(state_dir), "admin_config_get", {"name": "network"})
...
async def test_config_get_public_hashes_names(state_dir: Path) -> None:
    result = await _call(
        StateStore(state_dir), "admin_config_get", {"name": "users", "level": "public"}
    )
    payload = _json_block(result)
...
async def test_config_get_raw_denied_by_default(
    ...
    monkeypatch.delenv("ECO_ADMIN_ALLOW_RAW", raising=False)
    result = await _call(
        StateStore(state_dir), "admin_config_get", {"name": "discord", "level": "raw"}
    )
    assert result.isError is True
    payload = _json_block(result)
...
async def test_config_get_unknown_name_is_error(state_dir: Path) -> None:
    result = await _call(StateStore(state_dir), "admin_config_get", {"name": "../../secrets"})
    assert result.isError is True
...
async def test_config_get_unknown_level_is_error(state_dir: Path) -> None:
    result = await _call(
        StateStore(state_dir), "admin_config_get", {"name": "network", "level": "god"}
    )
    assert result.isError is True
...
async def test_save_status_tool(state_dir: Path) -> None:
    result = await _call(StateStore(state_dir), "admin_save_status", {})
    payload = _json_block(result)
...
async def test_backup_list_tool_hashes_names_at_public(state_dir: Path) -> None:
    result = await _call(StateStore(state_dir), "admin_backup_list", {"level": "public"})
    payload = _json_block(result)
    assert payload["count"] == 3
...
async def test_log_tools_are_operator_only_and_strip_secret_values(state_dir: Path) -> None:
    store = StateStore(state_dir)
    denied = await _call(store, "admin_log_tail", {"stream": "web", "level": "public"})
    assert denied.isError is True
    payload = _json_block(await _call(store, "admin_log_tail", {"stream": "web", "lines": 3}))
    ...
    grep = _json_block(await _call(store, "admin_log_grep", {"stream": "web", "query": "TOKEN"}))
...
async def test_mods_and_world_tools(state_dir: Path) -> None:
    store = StateStore(state_dir)
    world = _json_block(await _call(store, "admin_world_meta", {}))
    assert world["metadata"]["MeteorImpactDays"] == 30
    mods = _json_block(await _call(store, "admin_mods_installed", {}))
    assert mods["count"] == 1
...
async def test_runtime_tools_use_fixed_info_route_and_redact_names(state_dir: Path) -> None:
    ...
    health = _json_block(
        await _call(StateStore(state_dir), "admin_service_health", {}, runtime=runtime)
    )
