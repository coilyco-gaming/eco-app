...
`get_government` (the `/info` law/title card) is the *current-state*
...
- **`get_civics` MCP tool** - `src/eco_mcp_app/civics.py` + `server.py` wiring. Returns a markdown summary + the structured `CivicsReport.to_dict()` JSON. Requires an admin API key server-side (`ECO_ADMIN_API_KEY`, SSM in the homelab deploy).
...
- **Laws-in-effect** - not derivable from the action stream (there is no per-law event). The surface points at `get_government` / `/info` for the active-law list rather than fabricating one.
