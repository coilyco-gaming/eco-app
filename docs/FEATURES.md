...
- **React frontend** - `frontend/`, a Vite + React + TypeScript SPA (no SSR). The Docker image builds it in a node stage and the fused service serves it at `/` (assets under `/assets`); a checkout without a build keeps the old `/` -> `/preview` redirect. The landing page renders a live world snapshot from `/preview.json` (meteor countdown, players, world stats, 60s poll) with a graceful degraded state, routed via react-router for the per-feature pages to come ([#2](https://forgejo.coilysiren.me/coilyco-gaming/eco-app/issues/2)). Scaffolding: eslint + vitest with component tests, run in CI. Dev verbs: `frontend-install` / `frontend-dev` / `frontend-build` / `frontend-test` / `frontend-lint`.
