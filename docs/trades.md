...
the economy card (`get_eco_economy`, `/economy`) only consumes aggregate
...
- **Price-over-time** - unit price = `CurrencyAmount / NumberOfItems`, mean per in-game day, for the busiest items. Rendered as an inline SVG line on `/trades`. This "falls out almost for free" once the rows are parsed.
