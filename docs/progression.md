...
- **The progression layer of `/jobs`** - `frontend/src/pages/Jobs.tsx`, consuming `/preview/progression.json`. Product UX lives here (the Jinja card is only the in-chat fragment). There is no standalone `/progression` page: [#90](https://forgejo.coilysiren.me/coilyco-gaming/eco-app/issues/90) merged it into `/jobs` so a reader sees one "who does what, and how they got there" surface. Two integrations share the one fetch: each current-roster **player card** gains a best-effort "how they got here" expandable timeline joined to the progression citizen by name, and a server-wide **"How the world got here"** block below the roster carries the per-day trend small-multiples and the most-gained-specialty / busiest-leveler / class-completion leaderboards. The fetch is best-effort - a failure or thin server leaves the current-state jobs tables exactly as they were, and the progression block and per-player timelines simply don't render. The old `/progression` route redirects to `/jobs`.
...
- **Server-wide trends** - per-in-game-day counts of each event kind (specialties gained, level-ups, professions, classes, enrollments), rendered as single-hue small-multiples in the `/jobs` progression block.
